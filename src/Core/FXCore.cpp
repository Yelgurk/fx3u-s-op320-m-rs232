...
void FXCore::checkAutoStartup(bool force_off)
{
    ...
    {
        if (in_range && !force_off && !prog_state->getState())
    ...
    {
        if (in_range && !force_off && !prog_state->getState() && !is_task_heating_running && !is_task_freezing_running && !is_task_heating_extra)
        {
